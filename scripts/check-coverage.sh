...
MODULE_PATH="github.com/tight-line/gatekeeper"
...
if [ -n "$ERRORS" ]; then
    echo "ERROR: Uncovered code without coverage:ignore comments:"
    echo ""
    echo -e "$ERRORS" | sort -u | grep -v "^$"
    echo ""
    echo "Either add tests or mark with: // coverage:ignore - <reason>"
    exit 1
fi
